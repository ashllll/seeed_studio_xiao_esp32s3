...
void loop() {
  ...
  if (now - lastUpdate >= UPDATE_INTERVAL) {
    ...
    if (!checkSensorConnection()) {
      displayError(PSTR("Sensor disconnected"));
      startHeating = false;
    } else {
      float temperature = max31865.temperature(100.0, 430.0);
      
      if (checkOverheat(temperature) || checkTemperatureChange(temperature) || checkHeatingElement()) {
        startHeating = false;
      }

      if (startHeating) {
        if (now - heatingStartTime > HEATING_TIMEOUT) {
          displayError(PSTR("Heating timeout"));
          startHeating = false;
        } else {
          input = temperature;
          myPID.Compute(); // 计算PID输出
          if (temperature >= MAX_HEATING_TEMPERATURE) {
            output = 0;
          }
          ledcWrite(1, output); // 设置PWM输出
...
bool checkOverheat(float temperature) {
  if (temperature > OVERHEAT_THRESHOLD) {
    displayError(PSTR("Overheat detected"));
    return true;
  }
  return false;
}
...
bool checkTemperatureChange(float temperature) {
  if (abs(temperature - lastTemperature) > MAX_TEMPERATURE_CHANGE) {
    displayError(PSTR("Abnormal temp change"));
    return true;
  }
  return false;
}
...
bool checkHeatingElement() {
  unsigned long now = millis();
  if (startHeating && now - lastValidTemperatureTime > 5000 && input < setpoint - 5.0f) {
    displayError(PSTR("Heating element fault"));
    return true;
  }
  return false;
}
