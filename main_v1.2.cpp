...
void displayError(ErrorState error);
...
if (!max31865.begin(MAX31865_3WIRE)) {
  displayError(ErrorState::SensorDisconnected);
  errorState = true;
}
...
if (now - lastUpdate >= UPDATE_INTERVAL) {
  ...
  if (!checkSensorConnection()) {
    currentError = ErrorState::SensorDisconnected;
    errorState = true;
    startHeating = false;
  } else {
    ...
    if (checkOverheat(temperature)) {
      currentError = ErrorState::OverheatDetected;
      errorState = true;
      startHeating = false;
    } else if (checkTemperatureChange(temperature)) {
      currentError = ErrorState::AbnormalTempChange;
      errorState = true;
      startHeating = false;
    } else if (checkHeatingElement()) {
      currentError = ErrorState::HeatingElementFault;
      errorState = true;
      startHeating = false;
    } else if (startHeating && (now - heatingStartTime > HEATING_TIMEOUT)) {
      currentError = ErrorState::HeatingTimeout;
      errorState = true;
      startHeating = false;
    }
    ...
    if (startHeating && !errorState) {
      ...
      if (input >= MAX_HEATING_TEMPERATURE) {
        output = 0; // 立即停止加热
      }
...
if (now - lastAdjustTime >= 10000) { // 每隔10秒调整一次
  lastAdjustTime = now;
...
u8g2.setCursor(0, 25);
u8g2.print(messages[static_cast<int>(error)]);
u8g2.sendBuffer();
}
...
if (testTemp < 0 || testTemp > MAX_HEATING_TEMPERATURE) {
  displayError(ErrorState::AbnormalTempChange);
  return;
}
